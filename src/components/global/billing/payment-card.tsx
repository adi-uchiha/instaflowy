...
const PaymentCard = ({ current, label, landing }: Props) => {
	return (
		<div className={cn(
			...
			'p-[2px] rounded-xl overflow-hidden flex-1'
		)}>
			<div
				className={cn(
					landing && 'bg-pink-400',
					'flex flex-col h-full rounded-xl pl-5 py-5 pr-10 bg-background/95'
				)}
			>
				{
					landing ? (
				...
				{label === "PRO" ? (
					<p className='mb-2'>
						<b className='text-xl'>$2.99</b>/month
					</p>
				) : (
				...
				{PLANS[label === 'PRO' ? 1 : 0].features.map((i) => (
					...
					>
						<CircleCheckIcon className='text-indigo-500' />
						{i}</p>
				))}
				{landing ? (
					<Button 
